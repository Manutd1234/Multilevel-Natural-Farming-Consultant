...
function openaiModelCandidates() {
  ...
  // gpt-5.4 is the default (much stronger than 4o-mini); gpt-4o is the fallback.
  // An OPENAI_MODEL/CHATGPT_MODEL env value (a real model name) overrides the default.
  return uniqueNonEmpty([
...
function coerceList(value, fallback = []) {
  if (Array.isArray(value)) {
    ...
    return cleaned.length ? cleaned : fallback; // empty array → use the fallback
  }
...
// Map any numeric to a 0..1 confidence. Decimals (0..1) pass through; a slight
// overshoot (1..5, e.g. 1.5) clamps to 1; 5..100 is treated as a percentage
// (82 → 0.82); anything else is invalid.
function numericToConfidence(n) {
...
function coerceConfidence(value, fallback = 0.6) {
  if (typeof value === "number") {
    const c = numericToConfidence(value);
  ...
  const c = numericToConfidence(parseFloat(text.replace(/[^\d.]/g, "")));
  return c === null ? fallback : c;
}
